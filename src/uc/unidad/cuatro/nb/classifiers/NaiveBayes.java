...
package uc.unidad.cuatro.nb.classifiers;

import uc.unidad.cuatro.nb.dataobjects.Document;
import uc.unidad.cuatro.nb.dataobjects.FeatureStats;
import uc.unidad.cuatro.nb.dataobjects.NaiveBayesKnowledgeBase;
import uc.unidad.cuatro.nb.features.FeatureExtraction;
import uc.unidad.cuatro.nb.features.TextTokenizer;
...
public class NaiveBayes {
    private double chiCuadradaCriticalValue = 10.83; //equivalent to pvalue 0.001. It is used by feature selection algorithm
    ...
    public double getChiCuadradaCriticalValue() {
        return chiCuadradaCriticalValue;
    }

    public void setChiCuadradaCriticalValue(double chiCuadradaCriticalValue) {
        this.chiCuadradaCriticalValue = chiCuadradaCriticalValue;
    }
    ...
    private FeatureStats selectFeatures(List<Document> dataset) {        
        ...
        Map<String, Double> selectedFeatures = featureExtractor.chiCuadrada(stats, chiCuadradaCriticalValue);
    ...
    public void train(Map<String, String[]> trainingDataset, Map<String, Double> categoryPriors) throws IllegalArgumentException {
        ...
        knowledgeBase.n = featureStats.getN(); //number of observations
        ...
        if(categoryPriors == null) { 
            ...
            knowledgeBase.c = featureStats.getCategoryCounts().size(); //number of cateogries
            knowledgeBase.setLogPriors(new HashMap<String, Double>());
            ...
            for(Map.Entry<String, Integer> entry : featureStats.getCategoryCounts().entrySet()) {
                ...
                knowledgeBase.getLogPriors().put(category, Math.log((double)count/knowledgeBase.n));
            }
        }
        else {
            ...
            if(knowledgeBase.c != featureStats.getCategoryCounts().size()) {
            ...
            for(Map.Entry<String, Double> entry : categoryPriors.entrySet()) {
                ...
                knowledgeBase.getLogPriors().put(category, Math.log(priorProbability));
            }
        }
        ...
        for(String category : knowledgeBase.getLogPriors().keySet()) {
            for(Map.Entry<String, Map<String, Integer>> entry : featureStats.featureCategoryJointCount.entrySet()) {
                ...
                knowledgeBase.getLogLikelihoods().get(feature).put(category, logLikelihood);
            }
        }
    ...
    public String predict(String text) throws IllegalArgumentException {
        if(knowledgeBase == null) {
            throw new IllegalArgumentException("Bases de conocimientos perdidas: Make sure you train first a classifier before you use it.");
        }
        ...
        for(Map.Entry<String, Double> entry1 : knowledgeBase.getLogPriors().entrySet()) {
            ...
            for(Map.Entry<String, Integer> entry2 : doc.getTokens().entrySet()) {
                ...
                logprob += occurrences * knowledgeBase.getLogLikelihoods().get(feature).get(category); //multiply loglikelihood score with occurrences
            }
