...
package uc.unidad.cuatro.nb.features;
...
public class FeatureExtraction {
    ...
    public FeatureStats extractFeatureStats(List<Document> dataset) {
        ...
        for(Document doc : dataset) {
            stats.setN(stats.getN() + 1); //increase the number of observations
            ...
            if(categoryCount==null) {
                stats.getCategoryCounts().put(category, 1);
            }
            else {
                stats.getCategoryCounts().put(category, categoryCount+1);
            }
            
            for(Map.Entry<String, Integer> entry : doc.getTokens().entrySet()) {
     ...
     * @return 
     */
    public Map<String, Double> chiCuadrada(FeatureStats stats, double criticalLevel) {
        ...
        for(Map.Entry<String, Map<String, Integer>> entry1 : stats.featureCategoryJointCount.entrySet()) {
            ...
            for(Map.Entry<String, Integer> entry2 : categoryList.entrySet()) {
                ...
                N01 = stats.getCategoryCounts().get(category)-N11; //N01 is the total number of documents that do not have the particular feature BUT they belong to the specific category
                ...
                chiCuadradaScore = stats.getN() * Math.pow(N11*N00-N10*N01, 2)/
                                 ((N11+N01)*(N11+N10)*(N10+N00)*(N01+N00));
                
                //if the score is larger than the critical value then add it in the list
                if(chiCuadradaScore >= criticalLevel) {
                    previousScore = selectedFeatures.get(feature);
                    if(previousScore==null || chiCuadradaScore > previousScore) {
                        selectedFeatures.put(feature, chiCuadradaScore);
                    }
                }
            }
        }
